      ...
      <div className="grid grid-cols-12 gap-5">
        ...
        <div className="col-span-12 lg:col-span-4 flex flex-col gap-4">
          {[
            ...
            <Link key={i} to={s.href} className="bg-white p-6 rounded-xl border shadow-sm flex items-center justify-between hover:-translate-y-0.5 hover:shadow-md transition-all cursor-pointer" style={{ borderColor: `${PRIMARY}15` }}>
              <div>
                ...
                return (
                  <div key={i} className="flex gap-3">
                    ...
                    <div className="bg-[#f8f6f6] p-3 rounded-2xl rounded-tr-none flex-1">
                      <p className="text-xs font-bold text-[#181311] line-clamp-2">{
                        log.action === 'SEND_FAILED'
                          ? `כשל בשליחת הודעה (${log.metadata?.template_key || 'לא ידוע'})`
                          : (log.diff_summary && log.diff_summary.length > 80
                            ? log.diff_summary.substring(0, 80) + '…'
                            : log.diff_summary || `${log.action} — ${log.entity_name}`)
                      }</p>
